...
public class ColorEditDialogController {
	...
	private final ObservableList<ColorFamily> families = (ObservableList<ColorFamily>) colorFamilyController
			.listColorFamily();
	private final ObservableList<Base> bases = baseController.basesList();
	private final ObservableList<Coatings> coatings = coatingsController.coatingsList();
	...
	private void setBaseBox() {
		...
		FilteredList<Base> filteredItems = new FilteredList<>(bases, p -> true);
		...
		baseBox.setConverter(new StringConverter<>() {
	...
	private void setCoatingBox() {
		FilteredList<Coatings> filteredCoatings = new FilteredList<>(coatings, p -> true);
		coatingsBox.getEditor().textProperty().addListener((observable, oldValue, newValue) -> {
			...
			if (selected == null || !selected.toString().equals(editor.getText())) {
				Platform.runLater(() -> filteredCoatings.setPredicate(item -> {
		...
		coatingsBox.setConverter(new StringConverter<>() {
